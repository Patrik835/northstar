...
export function ConnectionsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSelected(null);
      setMessage(connection.status === "active" ? "Trading 212 connected and portfolio imported." : "Credentials encrypted and saved. This source is waiting for synchronization support.");
      await refresh();
  ...
  async function sync(connection: Connection) {
    ...
    try {
      ...
      setMessage(result.status === "active" ? "Trading 212 portfolio updated." : result.last_error || "Synchronization failed.");
      await refresh();
