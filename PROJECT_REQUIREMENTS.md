...
- Import both opening and closing sides of the documented trade history and create opening
  activity for current positions. eToro's Public API currently exposes no dividend-ledger
  field or endpoint, so dividend events must not be inferred from fees or P&L.
